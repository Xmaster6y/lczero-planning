...
def main(args: argparse.Namespace):
    ...
    for model in SMALL_MODELS:
        ...
        if args.combination_mode:
            hyp_it = iter(COMBINATIONS)
        else:
            hyp_it = iter([(random.choice(VALUES), random.choice(VALUES), random.choice(VALUES))])
        for alpha, beta, gamma in hyp_it:
            logger.info(f"Running with hyperparameters: {alpha}, {beta}, {gamma}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--use_argmax", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--max_moves", type=int, default=100)
    parser.add_argument("--combination_mode", action=argparse.BooleanOptionalAction, default=False)
