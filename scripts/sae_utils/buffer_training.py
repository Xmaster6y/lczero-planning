...
def sae_loss(
    ...
    out = ae(acts, output_features=True, ghost_mask=ghost_mask)

    f = out["features"]
    x_hat = out["x_hat"]
    mse_loss = t.nn.functional.mse_loss(x_hat, acts, reduction="none").sum(dim=1).mean()
    ...
    if ghost_mask is not None:
        ghost_loss = ghost_loss_fn(out, acts)
    else:
        ghost_loss = None
    ...
    if explained_variance:
        out_losses["explained_variance"] = explained_variance_score(
            acts.clone().detach().cpu().numpy(), x_hat.clone().detach().cpu().numpy()
        )
    if r2:
        out_losses["r2_score"] = r2_score(acts.clone().detach().cpu().numpy(), x_hat.clone().detach().cpu().numpy())
    return out_losses
...
def trainSAE(
    ...
    *,
    ...
    total_steps = n_epochs * max_train_batches
    ...
    for _ in range(n_epochs):
        ...
        for acts in train_buffer:
            ...
            if val_steps is not None and val_buffer is not None:
                if step % val_steps == 0:
                    eval(
                        ae,
                        val_buffer,
...
def eval(
    ...
    val_losses = {}
    len_val = 0
    for i, val_acts in enumerate(val_buffer):
